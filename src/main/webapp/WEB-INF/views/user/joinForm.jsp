		...
		$('#idCheck').on('click',function(){
			...
			$.ajax({
				...
				success : function(flag){
					if(flag){
						$('#idConfirm').html($('#id').val()+'는 사용 할 수 없는 아이디입니다').css('color','red');
						$('#id').val('').focus();
					}else{
						$('#idConfirm').html('이 아이디는 사용 가능 합니다').css('color', 'green');
					}
				},
				error : function(){
					alert("잘못 입력하셨습니다");
...
<table border="1" class="table table-bordered">
	<tr>
		...
		<td>
			<input type="text" name="userid" id="id" placeholder="ID중복확인 이용">
